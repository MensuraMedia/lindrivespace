...
| B5 | **First programmatic expand of an unpopulated folder ends up collapsed.** `expand_row()` returns True, `row-expanded` populates 72 children (dummy replaced), yet `row_expanded()` is False afterwards; a second `expand_row()` sticks. Most likely GTK collapses the row at the instant `populate()` removes the dummy child before the real children are inserted. Seen while scripting screenshots; check whether a user's first click on an expander shows the same flicker. | Screenshot script 2026-09-15: attempt 0 → collapsed, attempt 1 → expanded. | Open. In `ScanTreeModel.populate`, insert the real children *before* removing the dummy (or reuse the dummy row as the first child). |
...
| C3 | **[fixed 2026-09-15 — forked child]** **Snapshot serialisation after every scan** takes 4–11 s of CPU on a thread (GIL) and writes up to tens of MB (`/mnt/data`). | Log: "snapshot saved" 8–11 s after "scan done". | Serialise in a child process (see A6); skip snapshots for trees > 2 M entries or store only depth ≤ 6. | S |
